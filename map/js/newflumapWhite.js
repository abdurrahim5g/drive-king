...
function initializeSelectionList(listItems, dropDownId, defaultValue) {
    ...
    // d3.select(dropDownId).on('click', function() {
    //     var id = d3.select(this).select("ul").attr("id");
    //     var value = d3.select(this).select("ul").property("value");

    //     d3.selectAll(".gallery-drop-down")
    //         .each(function () {
    //             var selectionId = d3.select(this).select("ul").attr("id");
    //             if (selectionId !== id) {
    ...
    //         });
    // });
    $(dropDownId + " ul li").click(function(e) {
        $(this).parents('ul.dropdown_portfolio').children('li').removeClass('active');
        $(this).addClass('active');
        var value = $(this).attr('value');
        // $(dropDownId + " .response-menu span").html($(this).children('div').html());
        updateGalleryUpgraded(value);
    })
    $(".response-menu").click(function(e) {
        $(this).parents(dropDownId).children('ul.dropdown_portfolio').hasClass('active') ? 
            $(this).parents(dropDownId).children('ul.dropdown_portfolio').removeClass('active') : 
            $(this).parents(dropDownId).children('ul.dropdown_portfolio').addClass('active');
...
function renderGalleryFilters(impactAreas, productTypes, productActive, techLevels, countries) {   // updated at Feb 11th 2020
    initializeSelectionList(impactAreas.sort((a,b) => (a.name > b.name) ? 1 : -1), "#zoomdrop-impact-area-drop-down", "All Programs");
    initializeSelectionList(productTypes.sort((a,b) => (a.name > b.name) ? 1 : -1), "#zoomdrop-product-type-drop-down", "All Products");
    ...
    initializeSelectionList(productActive, "#zoomdrop-product-drop-down", "Active/Inactive");   // added at Feb 11th 2020
...
function updateDotInformation(dotData, dotType) {
    ...
    if (dotType === 'accelerators') {
        ...
        tipImg = dotData.logofile ? "<img src='images/AccelLogos/" + dotData.logofile + "' alt='" + dotData.Name + "' />" : "";
        ...
        if (dotType === "products") {
            tipImg = dotData.logofile ? "<img src='images/ProductLogos/" + dotData.logofile + "' alt='" + dotData.Name + "' />" : "";
        } else {
            tipImg = dotData.logofile ? "<img src='images/TechLogos/" + dotData.logofile + "' alt='" + dotData.Name + "' />" : "";
        }
...
function freshdots(dotdata, dottype, isAsides) {
    ...
    circle.on("mouseenter", function (d) {
        ...
        if (dottype == "accelerators") {
            ...
            tipImg = d.logofile ? "<img src='images/AccelLogos/" + d.logofile + "' alt='" + d.Name + "' />" : "";
            ...
            if (dottype == "products") {
                tipImg = d.logofile ? "<img src='images/ProductLogos/" + d.logofile + "' alt='" + d.Name + "' />" : "";
            } else if (dottype == "technologies") {
                tipImg = d.logofile ? "<img src='images/TechLogos/" + d.logofile + "' alt='" + d.Name + "' />" : "";
            }
        }
...
function zoomed() {
    ...
    if (transk)
        wholemap.style("stroke-width", 1 / transk + "px").attr("transform", d3.event.transform);
...
function makeGalleryUpgrade(data) {
    ...
    var output = "<div class='content-products'><div class='row equal product-list'>";
    if (data.length == 0) {
    ...
    productListDisplay.forEach(function(d, key) {
            var site = d.Website ? getInlineDescription("Web",
                "<a href='" + d.Website + "' target='_blank' onclick='return confirmExit()'> " + d.Website + "</a>") : '';
            var contact = d.Contact ? getInlineDescription("Contact",
                "<a href='mailto: " + d.Contact + "'>" + d.Contact + "</a>") : '';
            ...
            var costShare = d.CostShare ? getDescription("Cost Share", "$" + numberWithCommas(d.CostShare)) : '';
            var productType = d.ProductType ? getDescription("Deliverable Type", d.ProductType) : '';
            var tipImg = d.logofile ? "<img class='gallerylogos img-fluid' src='images/ProductLogos/" + d.logofile + "' width='100%' alt='" + d.Name + "' style='height: 100%'/>" : "";
            ...
            var productTarget = escapeProductTarget(d.Name + d.ProductName).replaceAll(" ", "-");
            var classProductType = "product-type-" + d.ProductType.replaceAll(" ", "_").replace('/', '_');
            var classProductType2 = "product-type-" + d.ProductType2.replaceAll(" ", "_").replace('/', '_');
            var classProductActive = "product-active-" + d.Active;
            var classCountry = "product-country-" + d.Country.replaceAll(" ", "_");
            var classImpactArea = "product-impact-area-" + d.ImpactArea.replaceAll(" ", "_");
            var classImpactAreas = "";
            d.impactAreas.map(area => {
                classImpactAreas += (" " + "product-impact-area-" + area.replaceAll(" ", "_"));
            })
                "product-impact-area-" + d.ImpactArea.replaceAll(" ", "_");
            var classTRL = "product-TRL-" + d.Stage.replace(/^(TRL )?([^ ]+)/, '$2');

            // updated at Feb 11th 2020
            output = output +
                "<div class='productbox product-active-all " + classProductType + " " + classProductType2 + " " + classProductActive + " " + classCountry
                    + " " + classImpactAreas + " " + classTRL + "'><a href='../portfolio.html?id=" + key + "&type=" + (d.Active == "Y" ? 0 : 1) + "&p=All' class='readmore_btn'>" +
                    "<div class='product-content'><div class='image-area'>" +
                        // "<a data-toggle='modal' data-target='#" + productTarget + "' href='#' style='color:#000000;'>" +
                        tipImg + 
                        "</div><div class='ellipsis white-board'><p class='mb-0'><span class='galleryproductname'>" + d.Name + "</span></p>" +
                        "</div></div>" + "<div id='div-shadow'></div></a></div>";
...
function getDescription(label, description) {
	return "<div class='tipfield'><p><span class='tiplabel'>" + label + ": </span>" +
				description + "</p></div>";
}
...
function showProductsByStatus(status) {
	$(".productbox").show();
	if (status == "Active") {
		$(".productbox.product-active-N").hide();
	} 
	if (status == "InActive") {
		$(".productbox.product-active-Y").hide();
	}
}
